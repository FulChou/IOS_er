...
## 3.3 队列：
具有操作约束的线性表，只能在一端插入，而在另一段删除；

- 队列的顺序存储实现：**循环队列**

通常由一个一维数组和一个记录队列头元素的变量front以及纪律队列尾元素的变量rear组成

front 指向头一个元素，再前面一个；比如index -1

添加一个元素，front++；删除一个 rear+1；

**循环队列**： front==rear 是 没有元素，还是满的？
