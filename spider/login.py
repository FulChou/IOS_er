...
def get_login_id():
    ...
    req = session.post(url, post_data, headers,) # 不让重定向   
    # print('location: ', req.headers['Location']) 
    html = req.text
    print('req', req.cookies, req.headers,req.status_code,req.history,req.url ) # html 里面已经没有了 takenid：
    # print('\n',html)
    url2 = 'http://ca.its.csu.edu.cn/SysInfo/SsoService/69'
    
    req2 = session.get(url=url2,allow_redirects=True)
    print('url2: ',req2.text,req2.status_code,req2.headers,req2.cookies,req2.history,req2.url)
    # cookiesdict = requests.utils.dict_from_cookiejar(req.cookies)
    # print(cookiesdict)
    bf = BeautifulSoup(html, 'lxml')
    # name 是函数签名，所以不能用name = "tokenId"
    # tokenId_input_list = bf.find_all('input', attrs={'name': 'tokenId'})
    # print(tokenId_input_list)
    # tokenId = tokenId_input_list[0].get('value')
    return tokenId

# 登录到用户的界面：
def login(tokenId) -> str:
    url = 'http://202.197.71.125/loginsso.jsp'
    p_data = {
        "tokenId": tokenId,
        "account": username,
        "Thirdsys": "xsgzxt"  # xsgzxt
    }
    new_headers = {
    ...
    #     'Referer': 'http://ca.its.csu.edu.cn/',
    #     'User-Agent': userAgent,
    #     'Cookie': 'JSESSIONID=JSESSIONID=0249285FE2CB36F1E36E91AE923C798F',
    #     'Origin': 'http://ca.its.csu.edu.cn',
    #     'Host': '202.197.71.125',
    #     'Pragma': 'no-cache',
    ...
    # req = session.post('http://202.197.71.125/loginsso.jsp',
    #                    p_data, new_headers)  # 改用session会话
    req = session.post(url=url,data=p_data,headers=new_headers)  # 改用session会话
...
def get_stu_information():
    ...
    req = session.get(
        'http://202.197.71.125/a/physicalhealth/gxxl07/gXXL0704/form?type=add')
    # 拿出同学的数据：
...
if __name__ == "__main__":
    ...
    # tokenId = '77f0c73c096040c28e78b8447d74ad91'
    # login(tokenId)
